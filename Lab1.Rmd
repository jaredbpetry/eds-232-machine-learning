...
*Question:* Create a scatter plot using price on the y-axis and day on the x-axis.
...
*Question* In the first section of the chunk below, use a combination of dplyr::filter() and stringr::str_detect() to achieve what we want.
...
*Question:* Is this a useful plot 🤷? Does anything about it surprise you?

How do we make it useful? To get charts to display useful data, you usually need to group the data somehow.

*Question:* Within new_pumpkins, group the pumpkins into groups based on the month column and then find the mean price for each month (in the next chunk).
...
```
*Question:* Now do that again, but continue on and plot the results with a bar plot 
```{r}
...
*Question:* From looking at the baked_pumpkins tibble, how many total cities are represented in the data set?
...
*Question:* Calculate the correlation between pumpkin price and two other variables in the data set

```{r}

```

*Question:* Which of these three variables is most highly correlated with price? Why might this be?
...
*Question* What issues do you see with fitting a linear regression to this data?
